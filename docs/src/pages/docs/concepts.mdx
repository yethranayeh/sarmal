...
import SurgicalDemo from "../../components/SurgicalDemo.astro";
...
Sarmal instances are created with `createSarmal()` (canvas) or `createSarmalSVG()` (SVG). By default, they start animating immediately — you don't need to call `play()` unless you explicitly opt out.
...
<SurgicalDemo concept="initial-t" />
