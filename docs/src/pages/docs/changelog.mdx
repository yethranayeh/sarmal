...
`CurveDef` now supports an optional `kind` field (`"parametric"` or `"drawn"`) to distinguish how a curve was created. `drawCurve()` sets `kind: "drawn"` automatically.

```ts
// Built-in parametric curves (kind omitted, defaults to "parametric")
import { rose3 } from "@sarmal/core"

// Built-in drawn curve
import { artemis2 } from "@sarmal/core"
// artemis2.kind === "drawn"

// User-created drawn curves get kind: "drawn" automatically
import { drawCurve } from "@sarmal/core"
const curve = drawCurve([[-0.5, 0.3], [0.2, -0.8], [0.7, 0.4]])
// curve.kind === "drawn"
```

The built-in <Link href="/docs/curves/artemis2">Artemis II</Link> curve is now a hand-drawn Catmull-Rom spline through 21 control points, replacing the previous simplistic approximation. It is the first built-in curve created with `drawCurve`.

The <Link href="/docs/curves">curve catalog</Link> and curve detail pages now also display a kind badge to distinguish a built-in curve's origin.

<Alert label="Breaking" variant="error">`drawCurve()` default name changed from `"custom"` to `"drawn"`. Also added an optional `name` parameter for custom display names.</Alert>
