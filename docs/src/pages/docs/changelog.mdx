...
Dot-matrix gradient colors are now interpolated in Oklab space. This will eliminate the gray midpoint that could appear with certain palettes. The `gradient-animated` style now cycles continuously through the palette instead of <span class="oscillating"><SplitText text="oscillating" className="inline-block transition-transform duration-200" /></span>.
...
The `<Sarmal>`, `<SarmalSVG>`, and `<SarmalDotMatrix>` components, the `sarmal`/`sarmalSVG`/`sarmalDotMatrix` actions, and the `useSarmal*` hooks accept `morphEasing` and `morphAlign`, forwarded to `morphTo` when the `curve` changes. `morphEasing` defaults to the core `easeInOutCubic` ease, so prop-driven morphs are eased automatically with no change required.
