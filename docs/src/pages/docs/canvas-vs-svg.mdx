...
<Alert variant="warning">
	Both renderers require a **DOM**. You may need to guard initialization in SSR frameworks. See <Link href="/docs/getting-started/#frameworks-and-ssr">Frameworks and SSR</Link>.
</Alert>
...
**Accessibility note:** Canvas only renders pixels, so screen readers see *nothing*. Consider adding a visually hidden label outside the canvas element:

```html
<canvas id="sarmal"></canvas>
<span class="sr-only">Loading…</span>
```

```ts
import { createSarmal, astroid, palettes } from "@sarmal/core";

const canvas = document.getElementById("sarmal") as HTMLCanvasElement;
const sarmal = createSarmal(canvas, astroid, {
...
- Pre-allocates one `<path>` DOM element per trail segment at construction time. This is because SVG is a **retained-mode** API. The browser owns a live scene graph, so creating and destroying elements mid-animation is expensive. Canvas is **immediate-mode**: every frame is a fresh paint with no persistent objects, so it has no equivalent overhead.
...
- The animation lives in a responsive container
...
// SVG: pass an SVG element
const svgSarmal = createSarmalSVG(svgElement, curve, options);
```
