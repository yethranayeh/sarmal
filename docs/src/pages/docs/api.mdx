...
Creates a `CurveDef` from hand-drawn control points. The points form a <Link href="/docs/concepts#closing-the-loop">closed loop</Link>. The last point connects back to the first, and the resulting curve passes smoothly through every point.
...
- Points can be anywhere within the normalized range. Values outside `[-1, 1]` are *valid* but may push the curve outside the visible area.
