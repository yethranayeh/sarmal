...
// Freeze in place — loop keeps running, t stops advancing
sarmal.setSpeed(0);
...
<Alert variant="warning">
`setSpeed(0)` freezes the animation but does **not** stop the loop. Use `stop()` if you need to cancel the requestAnimationFrame entirely and save CPU.
</Alert>

### `getSpeed()`

Returns the effective speed the engine is currently using. When `setSpeed()` has been called, returns that value. Otherwise returns `curve.speed` — the curve's natural rhythm.
...
**Cancellation:** Calling `setSpeed()`, `setSpeedOver()`, or `stop()` while a transition is in progress cancels it — the Promise rejects with `Error('Speed transition cancelled')`. A subsequent `setSpeedOver()` starts from the current interpolated speed.
...
<Alert variant="info">
The braking pattern — decelerate before stopping — is explicit:
```ts
await sarmal.setSpeedOver(0, 400); // decelerate over 400ms
sarmal.stop();                      // then kill the loop
```
</Alert>
...
8. **`setSpeed(0)` vs `stop()`**: `setSpeed(0)` freezes the animation but the RAF loop keeps running — the sarmal remains reactive. `stop()` cancels the loop entirely and saves CPU. They look identical on screen but have different resource implications.

9. **`stop()` during a speed transition**: The pending `setSpeedOver()` Promise rejects with `'Speed transition cancelled'`, but `userSpeedOverride` is *not* cleared. On `start()`, the animation does not resume with curve's default, but with the interpolated speed from when it had stopped.
  - Call `resetSpeed()` after `stop()` to return to the natural rhythm.
