...
Creates a `CurveDef` from hand-drawn control points. The points form a <Link href="/docs/concepts#closing-the-loop">closed loop</Link>. The last point connects back to the first, and the resulting closed Catmull-Rom spline passes smoothly through every point. The returned curve has `kind: "drawn"` set automatically.
...
- The returned `CurveDef` has `period: 2π` and `kind: "drawn"` and can be passed to `createSarmal`, `createSarmalSVG`, `morphTo`, or the lower-level engine constructor.
