...
<Alert variant="tip">The built-in <Link href="/docs/curves/artemis2/">Artemis II</Link> curve is a real-world example of a `drawCurve`-based curve — 21 control points defining a Catmull-Rom spline. Its source in `@sarmal/core/curves/artemis2` shows the pattern for making a drawn curve a reusable built-in.</Alert>
...
<ConceptDemo layout="bottom">
  ...
  <div slot="info">
    ...
    <span class="text-[0.62rem] text-muted">Stops the loop and removes the SVG. The instance cannot be restarted.</span>
  </div>
...
<Alert variant="info">
  Accepted color formats: `#rrggbb`, `#rgb`, `#rrggbbaa` (alpha stripped), `rgb()`, `rgba()` (alpha stripped). Note that, named colors (`"red"`), `hsl()`, and CSS keywords are not accepted.
</Alert>
