...
import {
  PACKAGE_SIZE,
  PACKAGE_SIZE_ENGINE,
  PACKAGE_SIZE_ALL_CURVES,
  PACKAGE_SIZE_DIFF,
  PACKAGE_SIZE_ENGINE_PCT,
  PACKAGE_SIZE_AUTOINIT_KB,
...
<Alert variant="warning" label="CDN-only">
  `auto-init.js` bundles all renderers, all curves, and DOM scanning code (~{PACKAGE_SIZE_AUTOINIT_KB} KB raw, unminified). It is designed exclusively for `<script src>` / CDN use.

  **Do not `import '@sarmal/core/auto'` inside a bundled application**. It will pull everything into your bundle and the side-effect initialization will run unconditionally.
</Alert>
...
- Drawn curves (like <Link href="/docs/curves/artemis2/">artemis2</Link>) carry the catmull-rom library as a real module import. Pure math curves (like <Link href="/docs/curves/rose3/">rose3</Link>) do not pull extra logic. So, catmull-rom is opt-in and only ships when you use a drawn curve.

> Measured output: **{PACKAGE_SIZE} KB gzip** for `createSarmal` + one curve.
...
Convenient for runtime curve selection or when you want all curves available. The tradeoff: because `curves` is a plain object assembling all {CURVE_COUNT} built-in curves, no bundler can eliminate the unused ones at build time.

> Measured output: **{PACKAGE_SIZE_ALL_CURVES} KB gzip**, which is roughly +{PACKAGE_SIZE_DIFF} KB gzip over a named import.
...
Each curve ships as a standalone file (~{CURVE_TYPICAL_SIZE_B} B gzip for `rose3`). The difference with named imports from the main entry is negligible when also importing a renderer. The only reason to reach for deep imports is when building a library that ships curve definitions with no renderer dependency at all. And I honestly don't know why you would need my <span class="underline decoration-wavy decoration-primary">curves</span>, but I don't judge.
...
> Measured output: **{PACKAGE_SIZE_ENGINE} KB gzip**, which is roughly {PACKAGE_SIZE_ENGINE_PCT}% of the full canvas renderer bundle.

<Alert variant='tip'>
  **In short:** use named imports from `@sarmal/core`. Tree-shaking works. The `curves` collection is fine when you need runtime selection; it costs +{PACKAGE_SIZE_DIFF} KB gzip over a single named curve.
</Alert>
