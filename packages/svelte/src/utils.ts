...
export function canvasInitValuesEqual(
  ...
  if (!initValuesEqual(a, b)) return false;
  if (!a || !b) return !a && !b;
...
export function extractRuntimeOptions(opts: Partial<SarmalOptions>): Partial<RuntimeRenderOptions> {
  ...
  if (opts.trailColor !== undefined) runtime.trailColor = opts.trailColor;
  if (opts.skeletonColor !== undefined) runtime.skeletonColor = opts.skeletonColor;
  if ("headColor" in opts) runtime.headColor = opts.headColor ?? null;
  if (opts.trailStyle !== undefined) runtime.trailStyle = opts.trailStyle;
  return runtime;
}
